# Merged Migration Plan for Admin System, Achievements, and Profile Creation Fix

## Problem Analysis

Based on reviewing migrations 12-17, the main issues are:

1. **Signup not working**: Multiple attempts to fix the `handle_new_user` trigger function
2. **RLS conflicts**: Row Level Security policies preventing trigger-based profile creation
3. **Achievements system**: Complex implementation with multiple iterations
4. **Data consistency**: Missing profiles for existing users

## Root Causes Identified

1. **RLS Policy Conflicts**: The trigger function runs with `auth.uid()` as NULL, causing RLS policies to block profile creation
2. **Incomplete Profile Fields**: Missing `is_admin` and `theme_preference` fields in early versions
3. **Constraint Violations**: Unique constraint violations when trying to create user_streaks records
4. **Function Complexity**: Overly complex trigger functions with poor error handling

## Merged Solution Approach

### 1. Simplified Profile Creation System

We'll create a bulletproof profile creation system with:

- A single, simple trigger function
- Permissive RLS policies for profile creation
- Proper error handling that doesn't break signup
- Service role permissions for trigger operations

### 2. Achievements System

We'll implement a comprehensive achievements system with:
...
As requested, we'll disable RLS for all tables by:
- Dropping all existing RLS policies
- Keeping tables accessible to all authenticated users

## Implementation Plan

### Part 1: Disable RLS for All Tables
...
-- Add similar DROP statements for all other policies
```

### Part 2: Create Simplified Profile Creation System
...
### Part 3: Implement Achievements System
...
Based on the analysis, the following files can be deleted as they are no longer needed:

1. `supabase/migrations/012_comprehensive_admin_achievements_fix.sql`
2. `supabase/migrations/013_fix_profile_creation_and_achievements.sql`
3. `supabase/migrations/014_debug_and_fix_profile_creation.sql`
4. `supabase/migrations/015_fix_trigger_and_rls.sql`
5. `supabase/migrations/016_fix_profile_streaks_conflict.sql`
6. `supabase/migrations/017_emergency_fix_signup.sql`
7. `supabase/test_emergency_fix.sql`
8. `supabase/test_profile_system.sql`

## Implementation Steps

1. Create the new merged migration file
2. Delete the old migration files (12-17)
3. Delete unnecessary SQL files from root folder
4. Run the database reset to apply the new migration
5. Verify the signup functionality works correctly